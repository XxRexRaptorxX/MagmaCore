...
public class ConfigHelper {
    ...
    /**
     * Sets a configuration category on the given builder, pushing a new comment header and Magma Core
     * language tag.
     *
     * @param builder the {@link ModConfigSpec.Builder} to configure
     * @param category the {@link ConfigCategories}; used both as the push key and to generate a
     *     comment
     */
    public static void setCategory(ModConfigSpec.Builder builder, ConfigCategories category) {
        ...
        builder.push(name)
                .comment(Character.toUpperCase(name.charAt(0)) + name.substring(1))
                .translation(category.getCategoryLangTag());
    }
    ...
    /**
     * Registers all provided configuration specs for this mod with the loader. Optionally places
     * config files in a subfolder matching the mod ID.
     *
     * @param container the {@link ModContainer} of the mod
    ...
    public static void registerConfigs(
            ...
            String modId,
            boolean withFolder,
            @Nullable ModConfigSpec serverConfig,
            @Nullable ModConfigSpec clientConfig,
            @Nullable ModConfigSpec commonConfig,
            @Nullable ModConfigSpec startupConfig) {
        String path = modId;
        ...
        if (startupConfig != null)
            container.registerConfig(ModConfig.Type.STARTUP, startupConfig, path + "-startup.toml");
     ...
     *
     * <p>This method returns {@code true} if either the MagmaCore debug mode is enabled or the
     * mod-specific debug mode is passed as {@code true}.
     *
     * @param modSpecificDebugMode a mod-specific flag indicating whether debug mode should be enabled
     * @return {@code true} if debug mode is enabled globally or for the specific mod; {@code false}
     *     otherwise
     */
